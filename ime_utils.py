...
def fix_entry_ime_font(hwnd_id: int,
    ...
    tkinter/ttk does not call this API automatically, so without it the IME
    falls back to the system default font and the two states look different.

    Usage (bind once in main.py for all TEntry widgets):
    ...
    try:
        ...
        # Use the screen DPI reported by GetDeviceCaps(hdc, LOGPIXELSY=90).
        u32   = ctypes.windll.user32
        ...
        lf_height = -int(point_size * dpi / 72)

        lf = LOGFONT()
        lf.lfHeight   = lf_height
        lf.lfWeight   = 400   # FW_NORMAL
        lf.lfCharSet  = 128   # SHIFTJIS_CHARSET (needed for Japanese IME)
